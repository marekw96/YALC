...
Response LedsConfigurationPage::handle(const Request &request)
{
    ...
    auto printPin = [&](uint32_t id) {
        response.write("PIN ").write(std::to_string(id))
                ...
                .write("<option value=\"BRG\"").write(app.ledsConfiguration->getColorConfig(id) == LedColorConfig::BRG?"selected/":"").write(">BRG</option>")
                .write("</select>")
                .write("<br />");
    };
...
void LedsConfigurationPage::handleChaningPixelsNumber(const Request &request, Response &response)
{
    ...
    else if(cc0->value == "BRG"){
        app.ledsConfiguration->setColorConfigFor(0, LedColorConfig::BRG);
    }
    else {
    ...
    else if(cc1->value == "BRG"){
        app.ledsConfiguration->setColorConfigFor(1, LedColorConfig::BRG);
    }
    else {
