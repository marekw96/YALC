...
class SensorBottomToTop(YALCAnimation):
	color = [20, 20, 20]
	status = CLOSED
	enabled = False
	timeToClose = 0
	...
	def periodic(self, timeElapsedInUs):
		...
		if self.status == DONE:
			...
			if self.lastPixel >= self.getNumberOfPixels():
				self.status = DONE
		...
		self._fillSize(self.color, self.lastPixel)

	def _fillSize(self, color, size):
		for i in range(size):
